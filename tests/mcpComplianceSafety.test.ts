...
describe("mcp diagnostics and packs", () => {
  test("question bank includes AMC-MCP-1..5", () => {
    ...
    expect(lookup.get("AMC-MCP-4")?.layerName).toBe("Skills");
    expect(lookup.get("AMC-MCP-5")?.layerName).toBe("Skills");
  });
  ...
  test("core policy packs expose mcp-safety", () => {
    ...
    expect(pack?.targetAdjustments["AMC-MCP-4"]).toBe(5);
    expect(pack?.targetAdjustments["AMC-MCP-5"]).toBe(5);
  });
