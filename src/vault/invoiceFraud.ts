...
export function checkInvoice(invoice: { amount: number; payee: string; reference?: string; dueDate?: string }): FraudCheckResult {
  const flags: string[] = [];
  let riskScore = 0;
  ...
  if (!invoice.reference) { flags.push('Missing reference number'); riskScore += 10; }
  if (/test|fake|dummy/i.test(invoice.payee)) { flags.push('Suspicious payee name'); riskScore += 25; }
  if (invoice.payee.length < 3) { flags.push('Payee name too short'); riskScore += 15; }
  if (invoice.dueDate) {
