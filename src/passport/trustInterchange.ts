...
export const TRUST_TRANSLATIONS: TrustTranslation[] = [
  {
    ...
    mappings: [
      { sourceDimension: "security", targetDimension: "MANAGE-3", conversionFactor: 0.8, offset: 10, confidence: 0.85 },
      { sourceDimension: "governance", targetDimension: "GOVERN-1", conversionFactor: 0.9, offset: 5, confidence: 0.90 },
      { sourceDimension: "evaluation", targetDimension: "MEASURE-2", conversionFactor: 0.85, offset: 8, confidence: 0.80 },
      { sourceDimension: "safety", targetDimension: "MAP-1", conversionFactor: 0.75, offset: 12, confidence: 0.75 },
      { sourceDimension: "transparency", targetDimension: "MANAGE-4", conversionFactor: 0.8, offset: 10, confidence: 0.82 },
    ],
  },
  {
    sourceSystem: "amc",
    ...
    mappings: [
      { sourceDimension: "governance", targetDimension: "A.6", conversionFactor: 0.9, offset: 5, confidence: 0.88 },
      { sourceDimension: "reliability", targetDimension: "A.8", conversionFactor: 0.85, offset: 7, confidence: 0.82 },
      { sourceDimension: "evaluation", targetDimension: "A.9", conversionFactor: 0.88, offset: 6, confidence: 0.85 },
      { sourceDimension: "safety", targetDimension: "A.10", conversionFactor: 0.82, offset: 9, confidence: 0.80 },
    ],
  },
];
...
export function translateTrustScores(
  ...
  for (const mapping of translation.mappings) {
    ...
    if (sourceScore !== undefined) {
      const translatedScore = Math.min(100, Math.max(0, sourceScore * mapping.conversionFactor + mapping.offset));
      result[mapping.targetDimension] = {
        score: Math.round(translatedScore * 10) / 10,
